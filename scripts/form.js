        ...
        if (typeof (FileReader) != "undefined") {
            ...
            reader.onload = function (e) { 
                ...
                for (var i = 1; i < result.length; i++) {
                    ...
                    var card = {}; 
                    card.type = "card";
                    card.title = result[i][0] + ". " + result[i][1];
...
async function getCardsOnBoard(csvArray) {  
    ...
    for (const row of csvArray) { 
        ...
        if (foundWidgets.length === 0) {
            console.log('no card with ID ' + row[0] + '. Exiting.');
            return;
        }
        ...
        await updateWidgets(foundWidgets, row);
...
async function getCardDetails(widgets) {
    ...
    let rows = [];
    rows.push(["ID", "Title", "Description", "Tag 1", "Tag 2", "Tag 3", "Tag 4", "Tag 5"]);
    ...
    for (widget of widgets) {
        ...
        if (card.length) {
            ...
            let title = card[0].title.split('. ');

            result = [ title[0], title[1], card[0].description ];
